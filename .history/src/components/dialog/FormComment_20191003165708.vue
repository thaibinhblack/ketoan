         ...
         <v-card>
          <v-toolbar dark color="primary">
            ...
            <v-card-text class="contnet-comment">
                <v-row>
                    <v-col class="item-comment" cols="12" sm="12" v-for="(comment,i) in comments" :key="i">
                        <v-avatar
                            size="50PX"
                            color="#e2e2e2"
                        >
...
export default {
    ...
    methods: {
        ...
        async apiGetComment()
        {
            ...
            await this.$http.get(this.$store.state.API_URL + 'comment?api_token='+this.$session.get('token')+'&UUID_FILE_MANAGEMENT='+this.file.UUID_FILE_MANAGEMENT).then((response) => {
