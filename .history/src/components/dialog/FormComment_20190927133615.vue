         ...
         <v-card>
          <v-toolbar dark color="primary">
            ...
            <v-card-text class="contnet-comment">
                <v-row>
                    <v-col class="item-comment active" cols="12" sm="12" v-for="(comment,i) in comments" :key="i">
                        <v-avatar
                            size="50PX"
                            color="#e2e2e2"
                        >
                            <img v-if="comment.AVATAR != null" :src="$store.state.PUBLIC_URL + comment.AVATAR" alt="alt">
                        </v-avatar>
                        <span style="margin-left: 15px;">{{comment.USERNAME}}</span>
                        
                        <p style="margin-top: 15px">{{comment.CONTENT_COMMENT}}</p>
                    </v-col>
                </v-row>
            </v-card-text>
...
export default {
    ...
    methods: {
        ...
        apiGetComment()
        {
            this.$http.get(this.$store.state.API_URL + 'comment?api_token='+this.$session.get('token')+'&UUID_FILE_MANAGEMENT='+this.file.UUID_FILE_MANAGEMENT).then((response) => {
