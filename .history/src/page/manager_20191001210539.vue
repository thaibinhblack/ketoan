...
export default {
    ...
    created()
    {
        this.$http.get(this.$store.state.API_URL + 'user?api_token='+Vue.$session.get('token')+'&get_data=1').then((response) => {
        ...
        messaging.requestPermission().then(function () {
            ...
            console.log(token);
            axios.get('http://127.0.0.1:8000/api/notify?api_token='+this.$session.get('token')+'&NOTIFY_TOKEN='+token)
        })
        .catch(function (err) {
