...
export default {
    ...
    methods: {
       async ApiGetFolder(year)
       {
            this.$sotre.state.loading = true
            await this.$http.get(this.$store.state.API_URL + 'folder?api_token='+this.$session.get('token')+'&year='+year).then((response) => {
