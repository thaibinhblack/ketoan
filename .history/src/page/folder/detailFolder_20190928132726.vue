<template>
    ...
    <form-add-folder :dialog="dialog_addfolder" @closeDialog="dialog_addfolder = $event" :folder="folder" :update="false"/>
    </v-row>
</template>
...
export default {
    ...
    methods:
    {
        ApiDetailFolder()
        {
             this.$http.get(this.$store.state.API_URL + 'file?api_token='+this.$session.get('token')+'&folder='+this.$route.params.uuid+'&month='+this.month).then((response) => {
        ...
        getFolder()
        {
            this.$http.get(this.$store.state.API_URL + 'folder/'+this.$route.params.uuid+'?api_token='+this.$session.get('token')+'&year=2019&UUID_PARENT='+this.$route.params.uuid)
            .then((response) => {
