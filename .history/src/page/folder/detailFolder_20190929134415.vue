<template>
    <v-row>
        ...
        <v-col cols="12" sm="8">
            <ul class="list-link">
                ...
                <li><router-link to="/manager/folders" style="color:#3333;"><v-icon>mdi-folder-open</v-icon> Danh sách thư mục</router-link></li>
                <li style="cursor:pointer;" v-for="(link,index) in list_folder" :key="index" @click="updateLink(link,index)"><v-icon>mdi-chevron-right</v-icon> <v-icon>mdi-folder-open</v-icon>{{link.NAME_FOLDER}}</li>
            </ul>
        </v-col>
    ...
    <pdf src="@/assets/pdf/0.-Quy-trình-tương-tác-Mô-tả-công-việc-của-Media-Thiết-kế-Truyền-thông.pdf" > </pdf>
    <form-upload-file 
    :dialog="dialog" 
