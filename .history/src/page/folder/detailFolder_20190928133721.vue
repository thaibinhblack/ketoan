...
export default {
    ...
    methods:
    {
        ...
        getFolder()
        {
            this.$http.get(this.$store.state.API_URL + 'folder/'+this.$route.params.uuid+'?api_token='+this.$session.get('token')+'&year='+this.folder.YEAR_FOLDER+'&UUID_PARENT='+this.$route.params.uuid)
            .then((response) => {
