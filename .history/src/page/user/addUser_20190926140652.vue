...
export default {
    ...
    methods: {
        ...
        checkUsername()
        {
            this.$http.get(this.$store.state.API_URL + 'user/check?api_token='+this.$session.get('token')+'&USERNAME='+this.user.USERNAME)
        },
        createUser()
        {
            this.$http.post(this.$store.state.API_URL + 'user?api_token='+this.$session.get('token'),data).then(() =>{
