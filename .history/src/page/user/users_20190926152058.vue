    ...
    <v-col cols="12" sm="12">
        <v-card>
            <v-card-title primary-title>
                <button class="btn-add"> <router-link to="/manager/add-user"><v-icon>mdi-account-plus</v-icon> Thêm người dùng mới</router-link></button>
            </v-card-title>
            <v-card-text>
                <v-data-table
                    :headers="headers"
                ...
                >
                    ...
                    <template v-slot:item.GENDER="{item}">
                        <v-icon title="Nam" v-if="item.GENDER == 1">mdi-gender-male</v-icon>
                        <v-icon  title="Nữ" v-else>mdi-gender-female</v-icon>
                    </template>
                     <template v-slot:item.ACTION="{item}"> 
                         <router-link :to="'/manager/user/'+item.USERNAME"> <v-icon>mdi-chevron-right</v-icon></router-link>
                    </template>
                </v-data-table>
            </v-card-text>
        </v-card>
