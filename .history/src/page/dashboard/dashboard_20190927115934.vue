...
<v-row>
    ...
    <v-col cols="12" sm="12" v-for="(parable,index), parables" :key="index">
        <v-card>
            <v-card-text>
...
export default {
    ...
    methods:{
        ...
        createParable()
        {
            ...
            this.$http.post(this.$store.state.API_URL + 'parable?api_token='+this.$session.get('token'),data).then((response) => {
