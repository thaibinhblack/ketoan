                ...
                <div class="list-notify">
                    <ul class="ul-notify" :class="{active: active}">
                        ...
                        <li v-for="(notify,i) in notifyData.data" :key="i">
                            <p><v-icon color="blue" v-if="notify.STATUS == 0">mdi-new-box</v-icon> Bạn được phân công quản lý thư mục {{notify.NAME_FOLDER}}</p>
    ...
    methods:{
        ...
        async notify()
        {
            ...
            await this.$http.get(this.$store.state.API_URL + 'manager/folder?api_token='+this.$session.get('token')+'&status=0').then((response) => {
                this.notifyData.number =  response.data.filter((value,index,array) => {
